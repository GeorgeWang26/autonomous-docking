...
def docking(event):
    global cx0, w, alpha, cam_pub, cam_msg, bot_pub, bot_msg, is_docking, phase_one, first_time, bot_cam_together
    if not is_docking:
        print("out")
        return
    if phase_one:
        if (0.45 * w < cx0) and (cx0 < 0.55 * w):
